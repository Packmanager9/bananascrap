        ...
        draw() {
            if(this.wheel == 1){
                if(typeof this.hub == 'undefined'){
                    this.hub = new Circle(this.x, this.y, this.wheelsize, "gray")
                    this.hub.draw()
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Car {
        constructor(){
            ...
            this.gravity = .2
            for(let t = 0;t<12;t++){
                this.angles.push(t*(Math.PI/6))
                let point = new Circle(350,350, 1, "#00FF00")
                point.dis = (Math.random()*35)+10
                point.wheelsize =  (Math.random()*17)+1
                point.wheel = 0
                if(Math.random()<.325){
                    point.wheel = 1
                }
        ...
        draw(){
            ...
            for(let t = 0;t<this.angles.length;t++){
                ...
                if(floor.doesPerimeterTouch(this.points[t])){
                    count++
                }else if(typeof this.points[t].hub !== 'undefined'){
                    if(floor.doesPerimeterTouch(this.points[t].hub)){
                        count++
                    }
                }
            }
            ...
            if(count == 1){
                ...
                for(let t = 0;t<this.angles.length;t++){
                    ...
                    if(floor.doesPerimeterTouch(this.points[t])){
                        ...
                        for(let k = 0;k<this.angles.length;k++){
                            if(this.points[k].x > this.points[t].x){
                                this.tilt-=this.points[k].dis*Math.cos(this.angles[k]+this.metaangle)
                            }else{
                                this.tilt+=this.points[k].dis*Math.cos(this.angles[k]+this.metaangle)
                            }
                        }
    
                        // this.metaangle-=(this.tilt/this.angles.length)
                    }
    
    
    
                    if(floor.y < (this.points[t].y)){
                        this.body.y -= (this.points[t].y-floor.y)*.55
                    }
    
    
    
                    if(typeof this.points[t].hub !== 'undefined'){
                        if(floor.y < (this.points[t].hub.y) + this.points[t].wheelsize){
                            this.body.y+=(floor.y-( (this.points[t].hub.y) + this.points[t].wheelsize))*.9
                        }
    
    
                        if(floor.doesPerimeterTouch(this.points[t].hub)){
                            this.body.ymom *= -.01
                            this.body.xmom += (this.points[t].hub.radius)*(Math.PI*.025)
                            this.tilt += 100
                        ...
                        for(let k = 0;k<this.angles.length;k++){
                            if(this.points[k].x > this.points[t].x){
                                this.tilt-=(this.points[k].dis*Math.cos(this.angles[k]+this.metaangle))*.1
                            }else{
                                this.tilt+=(this.points[k].dis*Math.cos(this.angles[k]+this.metaangle))*.1
                            }
                        }
                ...
                this.body.y += 1
            for(let t = 0;t<this.angles.length;t++){
                this.points[t].x = this.body.x + (Math.cos(this.angles[t]+this.metaangle)*this.points[t].dis)
                this.points[t].y = this.body.y + (Math.sin(this.angles[t]+this.metaangle)*this.points[t].dis)
            }
                ...
                for(let t = 0;t<this.angles.length;t++){
                    ...
                    if(typeof this.points[t].hub !== 'undefined'){
                        if(floor.y < (this.points[t].hub.y) + this.points[t].wheelsize){
                            this.body.y+=(floor.y-( (this.points[t].hub.y) + this.points[t].wheelsize))*.1
            ...
            this.metaangle-=(this.tilt)*.00001
            console.log((this.tilt)*.00001, this.metaangle)
            this.tilt *= .995
            this.body.move()
            this.body.draw()
            ...
            let hit = []
           let shape1 = castBetween(this.points[0], this.points[this.points.length-1], 10, 2)
           let link = new LineOP(this.points[0], this.points[this.points.length-1], "cyan", 2)
           link.draw()
            hit.push(shape1)
            for(let t = 1;t<this.angles.length;t++){
                let shape1 = castBetween(this.points[t], this.points[t-1], 10, 2)
                let link = new LineOP(this.points[t], this.points[t-1], "cyan", 2)
                hit.push(shape1)
                link.draw()
            }
            this.hitbox = new Shape(hit)
        }
    }

    let floor = new Rectangle(0, 500, 100000, 1000000, "#AAAAAA")
    let cart = new Car()
    ...
    class Shell{
        ...
        draw(){
            for(let t = 0;t<this.dots.length;t++){
                let point = new Point(this.dots[t].x + this.dots[t].xmom*7, this.dots[t].y + this.dots[t].ymom*7)
                this.dots[t].move()
                // this.dots[t].draw()
                let link = new LineOP(this.dots[t],point, "red", 3)
                link.draw()
            }
            let angle = Math.random()*Math.PI*2
            let dot = new Circle(this.center.x, this.center.y, 1, "red", Math.cos(angle), Math.sin(angle))
            this.dots.push(dot)
        }
    }
